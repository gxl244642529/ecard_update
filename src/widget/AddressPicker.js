...
const AddressPickerModule = require('react-native').NativeModules
  .AddressPickerModule;
...
export default class AddressPicker extends Component {
  ...
  _onRecvAddress = address => {
    this.props.onChange &&
      this.props.onChange(address === undefined ? null : address);
  };

  _renderLoading = () => {
    return (
      <View style={[styles.loading, this.props.style]}>
        <ActivityIndicator animating={true} style={globalStyles.loading} />
  ...
  _renderAddress = address => {
    return (
      <TouchableOpacity
        ...
        style={[styles.baseLoading, this.props.style]}
      >
