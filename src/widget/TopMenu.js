...
export default class TopMenu extends Component {
  ...
  onShow = index => {
    Animated.parallel([
      this.createAnimation(index, this.state.maxHeight[index]),
      this.createFade(1),
    ]).start();
  };
  ...
  renderList = (d, index) => {
    ...
    return (
      <Animated.View
        ...
        style={[
          styles.content,
          { opacity: enabled ? 1 : 0, height: this.state.height[index] },
        ]}
      >
  ...
  render() {
    ...
    for (var index = 0, c = this.state.top.length; index < c; ++index) {
      ...
      if (index > 0) {
        renderRows.push(<View key={'s' + index} style={styles.line} />);
      }
      renderRows.push(
        <TopMenuItem
          key={index}
    ...
    return (
      <View style={this.props.style ? this.props.style : styles.container}>
        <View style={styles.topMenu}>{renderRows}</View>
        ...
        >
          <Animated.View
            style={[styles.bg, { opacity: this.state.fadeInOpacity }]}
          />
