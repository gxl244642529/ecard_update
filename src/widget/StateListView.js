...
export const renderError = (error, isNetworkError, component) => {
  if (isNetworkError) {
    return (
      ...
      >
        <Image
          source={require('./images/ic_network_error.png')}
          resizeMode="contain"
    ...
    return (
      ...
      >
        <Image
          source={require('./images/ic_load_error.png')}
          resizeMode="contain"
          style={{ width: 100, height: 100 }}
        />
    ...
    return (
      <View style={styles.footer}>
        <Text style={styles.footerTitle}>
          {this.props.hasLoadAll ? '已加载全部' : '正在加载更多……'}
        </Text>
      </View>
    );
  }
...
export default class StateListView extends Component {
  static propTypes = {
    cancelLoad: PropTypes.bool, //是否在开始的时候加载
    renderEmpty: PropTypes.func,
    reserveProps: PropTypes.bool, //是否保留原来的数据
    paged: PropTypes.bool, //是否分页
    api: PropTypes.string.isRequired, //使用哪个分页
    data: PropTypes.object, //提交数据
    ...
    trigger: PropTypes.array, //触发的api
    triggerRefresh: PropTypes.bool, //触发是否刷新
  };
  ...
  _onReceivePage = result => {
    ...
    this.props.onChange &&
      this.props.onChange({ loading: false, list: list, page: result });
  };
  ...
  _onReceiveData = result => {
    ...
    if (this.props.paged) {
      ...
      this.props.onChange &&
        this.props.onChange({ loading: false, list: result });
    }
  };
  ...
  render() {
    let props =
      this.props.paged && !this.state.isLast
        ? {
            onEndReached: this._loadMore,
            onEndReachedThreshold:
              this.props.onEndReachedThreshold || onEndReachedThreshold,
          }
        : null;
...
function loadComplete(list) {
  return {
    ...
    dataSource:
      list.length > 0
        ? EMPTY_DATA_SOURCE.cloneWithRows(list)
        : EMPTY_DATA_SOURCE,
  };
}
