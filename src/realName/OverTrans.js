...
import {
  BANK_CARD_VERIFY_ERROR_MSG,
  BANK_CARD_VERIFY_TO_MANY_ERROR_MSG,
} from './RealI18N';
...
export default class OverTrans extends Component {
  ...
  render() {
    ...
    {
      data.flag === BANK_CARD_VERIFY_ERROR && (
        <BankMessage error={BANK_CARD_VERIFY_ERROR_MSG} />
      );
    }
    {
      data.flag === BANK_CARD_VERIFY_ERROR_TO_MUCH && (
        <BankMessage error={BANK_CARD_VERIFY_TO_MANY_ERROR_MSG} />
      );
    }
    return (
      <View style={[CommonStyle.container, styles.content]}>
        ...
        {data.realStatus === VERIFY_ERROR && (
          <VerifyProcessFail data={data.verifyError} />
        )}
        {data.realStatus === BANK_SEND_FAIL && (
          <NewMessageSubmit errorMsg={data.errorMsg && data.errorMsg} />
        )}
        ...
        {data.realStatus === BANK_CARD_VERIFY_ERROR_TO_MUCH && (
          <VerifyProcess />
        )}
