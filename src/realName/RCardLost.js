...
export default class RCardLost extends Component {
  ...
  _url = () => {
    console.log('链接到指定位置');
    WebUtil.open(
      Api.imageUrl + '/index.php/sys_agree_detail/content/reallost',
      ''
    );
  };
  // <FormInput
  ...
  _update = update => {
    ...
    if (update) {
      ...
      //勾选则卡号为原卡号、银行名称为原名称
      this.setState({
        bankCard: this.state.tmpbank,
        bankName: this.state.tmpbankName,
      });
      /**勾选则银行名称为原名称,卡号为原卡号*/
    }
  };
    ...
    return (
      <View style={[CommonStyle.container, container.container]}>
        ...
        <ScrollView>
          ...
          <MsgView
            tip="开通挂失时间"
            msg={self.createDate ? formatDate(self.createDate) : null}
          />
