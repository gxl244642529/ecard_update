...
export default class DatePickerItem extends Component {
  constructor(props) {
    super(props);
    this.state = {
      value: props.value ? props.value : new Date().format('yyyy-MM-dd'),
      initValue: null,
    };
  }
      ...
      <View>
        ...
        >
          ...
          <Text
            style={
              this.state.value
                ? [styles.holder, styles.holderDark, props.inputStyle]
                : styles.holder
            }
          >
            {this.state.initValue ? this.state.initValue : props.placeholder}
          </Text>
          {!props.noimg && (
            <Image
