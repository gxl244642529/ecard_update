...
export default class BankMessage extends Component {
  constructor(props) {
    ...
    state.error =
      (state.data && (state.data.error == '1' ? null : state.data.error)) ||
      props.error;
    this.state = state;
    this.form = new Form();
  }
  ...
  noErrorSubmit = () => {
    ...
    Api.api({
      ...
      timeoutMs: TIME_OUT,
      waitingMessage:
        self.idType == 1 && selfthis.isHXB == true ? MSG_IDCARD : MSG_OTHER,
  ...
  // {(this.state.data.idType==1&&this.state.isHXB)&&<View style={{flexDirection:'row'}}>
  //   <Text style={{paddingLeft:15,fontSize:12,color:'#ccc'}}>请输入银行预留手机号</Text>
  // </View>}
  // editable={this.state.isHXB?true:this.state.editable}
  render() {
    let progressImage = this.state.error
      ...
      <View style={[CommonStyle.container, container.container]}>
        ...
        <ScrollView>
          ...
          <FormSelect
            ...
            onChange={(index, bankId, bankName) => {
              this._banckCheck(bankId);
              this.setState({ bankName: bankName });
            }}
            hasBottom={true}
          />
          {this.state.isBankJian && (
            <FormSelect
              label="分行名称:"
              placeholder="请选择分行名称"
              ref="bankNo"
              ...
              onChange={(index, bankNo) => {
                this.setState({ bankNo: bankNo });
              }}
              hasBottom={true}
            />
          )}
          ...
          {this.state.data.idType == 1 &&
            this.state.isHXB && (
              ...
              >
                ...
                <Image
                  source={require('./images/huaxia.png')}
                  style={{
                    width: 18,
                    height: 18,
                    paddingLeft: 3,
