...
export default class NFCVertify extends Component {
    ...
    return (
      <View style={[styles.imgContainer, styles.center]}>
        <View style={{ top: 100, zIndex: -1000 }}>
          ...
          >
            <Image
              source={require('./images/anicard.png')}
              style={{ width: 90, height: 60, resizeMode: 'contain' }}
            />
          </Animated.View>
        </View>
        <Image source={require('./images/aniphone1.png')} style={styles.img}>
          {this.state.index % 3 == 1 && (
            <Image
              source={require('./images/nfc_wave1.png')}
              style={styles.img_wave1}
            />
          )}
          {this.state.index % 3 == 2 && (
            <Image
              source={require('./images/nfc_wave2.png')}
              style={styles.img_wave2}
            />
          )}
        </Image>
      </View>
  ...
  gotoOtherVertify = () => {
    ...
    Notifier.removeObserver('nfcTag', this.onNfc);
    let data = {
      cardId: this.state.data.cardId,
      fromto: this.state.data.fromto,
    };
